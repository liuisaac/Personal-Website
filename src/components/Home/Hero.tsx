...
const Hero = ({ ScrollPosition, SScrollPosition} : { ScrollPosition: number; SScrollPosition: Function; }) => {
    ...
    return (
        ...
        <div className={`w-full h-full bg-gray-900 text-white overflow-hidden relative`}>
            <div
            ...
            {/* Tags */}
            <div className={`flex-col w-1/32 items-start justify-center absolute z-10
             transition duration-300 ease-in-out select-none pointer-events-none
             ${( ScrollPosition > 0.05 ) ? "opacity-0" : "opacity-100"} $
      ...
      mt-[48vh] ml-[12vh] font-inconsolata text-[#9B9B9B] font-semibold tracking-wider"
                >
                    <span className="-mt-10 my-12">{`> BSc. Student @ UBC`}</span>
                    <span className="-mt-10 my-12">{`> Software Developer`}</span>
                    <span className="-mt-10">{`> Tech Enthusiast`}</span>
